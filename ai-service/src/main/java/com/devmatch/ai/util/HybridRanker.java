...
public class HybridRanker {
  public static List<RagChunk> fuse(List<RagChunk> vec, List<RagChunk> fts, int limit) {
    ...
    return byId.values().stream()
        .sorted((a, b) -> Double.compare(score.getOrDefault(b.id(), 0.0),
            score.getOrDefault(a.id(), 0.0)))
        .limit(limit)
        .toList();
  }

  private static Map<String, Integer> rank(List<RagChunk> l) {
    Map<String, Integer> m = new HashMap<>();
    for (int i = 0; i < l.size(); i++) {
      m.put(l.get(i).id(), i + 1);
    }
    return m;
  }
